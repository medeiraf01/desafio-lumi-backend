...
export class FaturasService {
  ...
  async processarArquivosPdf(files: Express.Multer.File[]) {
    ...
    for (const file of files) {
      ...
      const energia_eletrica = this.extractEnergyData(texto, 'Energia Elétrica');
      const energia_scee = this.extractEnergyDataOther(texto, 'Energia SCEE s/ ICMS');
      const energia_compensada = this.extractEnergyDataOther(texto, 'Energia compensada GD I' );
      
      // Extrair dados da fatura
      const clienteId = this.extrairClienteId(texto).split(' ')?.[0];
      const mesReferencia = this.extractValueBelowLabel(texto, 'Referente a')?.split(/\s+/).slice(0, 1).join(' ') ?? '';
      const numeroInstalacao = this.extractValueBelowLabel(texto, 'Nº DA INSTALAÇÃO')?.split(/\s+/).slice(1, 2).join(' ') ?? '';
      ...
      const faturaExtraida: CreateFaturaDto = {
        ...
        data_vencimento: this.extractValueBelowLabel(texto, 'Referente a')?.split(/\s+/).slice(1, 2).join(' ') ?? '',

        // Extrair Valor Total a Pagar
        total_a_pagar: this.toFloat(this.extractValueBelowLabel(texto, 'Referente a')?.split(/\s+/).slice(2, 3).join(' ')!),
        
        energia_eletrica_kwh: Number(energia_eletrica?.quantity || 0),
        energia_eletrica_valor: this.toFloat(energia_eletrica?.totalValue || "0"),
        ...
        energia_compensada_kwh: this.toFloat(energia_compensada?.quantity || "0"),
        energia_compensada_valor: this.toFloat(energia_compensada?.totalValue || "0"),
        
        // Extrair Contribuição de Iluminação Pública
        contrib_ilum_pub_municipal: this.toFloat(this.extract(texto, /Contrib Ilum Publica Municipal\s+(\d+,\d{2})/)) ?? 0,
       
        // Extrair Histórico de Consumo
        //historico_consumo: this.extrairHistoricoConsumo(texto),
       
        consumo_energia_eletrica_kwh: this.toFloat(energia_eletrica?.quantity || '0') + parseFloat(energia_scee?.quantity || '0'),
        
        valor_total_sem_gd: this.toFloat((energia_eletrica?.totalValue || '0') + parseFloat(energia_scee?.totalValue || '0') + parseFloat(this.extract(texto, /Contrib Ilum Publica Municipal\s+(\d+,\d{2})/))),
      };
  ...
  private extractEnergyData(text: string, includes: string): { 
    unit: string, 
    quantity: string, 
    unitPrice: string, 
    totalValue: string, 
...
private extractEnergyDataOther(text: string, includes: string): { 
  unit: string, 
  quantity: string, 
  unitPrice: string, 
  totalValue: string, 
  ...
  async findAll(filterDto: FindFaturasFilterDto) {
    ...
    const queryBuilder = this.faturaRepository.createQueryBuilder('fatura')
      .leftJoinAndSelect('fatura.cliente', 'cliente');
    
    // Aplicar filtro por número do cliente
    if (numero_cliente) {
      queryBuilder.andWhere('cliente.num_cliente = :numero_cliente', { numero_cliente });
    }
  ...
  private extrairClienteId(text: string): string {
    ...
    const lines = text.split('\n');
    const clienteIndex = lines.findIndex(line => line.includes('Nº DO CLIENTE'));
  ...
  private extrairHistoricoConsumo(text: string): { mesAno: string; consumo: number; media: number; dias: number }[] {
    const regex = /Histórico de Consumo\s*MÊS\/ANO\s*Cons\. kWh\s*Média kWh\/Dia\s*Dias([\s\S]*?)Reservado ao Fisco/;
    const match = text.match(regex);
    if (!match) return [];
    /*
    linhas.map(linha => {
      const [mesAno, consumo, media, dias] = linha.trim().split(/\s+/);
      return {
        mesAno,
        consumo: this.toFloat(consumo),
        media: this.toFloat(media),
